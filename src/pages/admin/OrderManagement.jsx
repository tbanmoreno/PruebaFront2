...
const OrderManagement = () => {
  ...
  const handleDownloadInvoice = async (order) => {
    // CORRECCIÓN CRÍTICA: Añadimos .trim() para eliminar espacios invisibles como "PAGADO "
    ...
    try {
      // Usamos la ruta corregida que definimos en el controlador
      ...
      notify.error("Error", "No se encontró factura física para este pedido.");
    } finally {
  ...
  if (isLoading) return (
    <div className="flex flex-col items-center justify-center p-40 space-y-4">
      ...
      <p className="text-stone-400 font-black uppercase tracking-widest text-xs">Sincronizando Logística...</p>
    </div>
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 p-4 md:p-8">
      <header>
        ...
        <p className="text-stone-400 font-bold text-xs uppercase tracking-widest">Gestión de Cosechas y Estados</p>
      </header>
        ...
        {orders?.map(order => (
          <div key={order.idPedido} className="bg-white rounded-[2.5rem] border border-stone-100 shadow-sm overflow-hidden hover:border-amber-400 transition-all">
            <div className="p-6 md:p-8 flex flex-col lg:flex-row items-center justify-between gap-8">
              ...
              <div className="flex flex-wrap items-center justify-end gap-8 w-full lg:w-auto">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className="p-4 bg-stone-900 text-amber-400 rounded-2xl hover:bg-amber-600 hover:text-stone-950 transition-all shadow-lg"
                  >
            ...
            {/* Renderizado de detalles corregido */}
